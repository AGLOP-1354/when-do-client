...
public struct TodoModel:Codable {
  let id:String, isCompleted: Bool, title: String;
}
...
struct Provider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        let userDefaults = UserDefaults(suiteName: "group.when.do.widget")
      ...
      ZStack{
        ...
        HStack(alignment: .center, spacing: 20){
          ...
          if entry.todos.count > 0 {
            ...
            VStack(alignment: .leading){
              Text("등록된 할 일이 없습니다.").font(.system(size:14)).foregroundColor(Color.gray)
              Text("이곳을 클릭하여 할 일을 추가해 보세요.").font(.system(size:14)).foregroundColor(Color.gray)
            }
          }
        }.padding()
      }
...
struct WhenDoWidget_Previews: PreviewProvider {
  ...
  var body: some View {
    HStack(alignment: .center){
      if todo.isCompleted {
        ...
        Text(todo.title).font(.system(size: 14)).foregroundColor(Color.gray).strikethrough(true).frame(width:140, alignment: .leading).lineLimit(1)
      }
      else {
        ...
        Text(todo.title).font(.system(size: 14)).foregroundColor(colorScheme == .dark ? Color(red:1.0, green: 1.0, blue: 1.0) : Color(red:0.0, green: 0.0, blue: 0.0)).frame(width:140,alignment: .leading).lineLimit(1)
      }
    }
    Spacer()
  }
